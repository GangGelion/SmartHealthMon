...
open class HeartRateOmeter {
    ...
    protected fun createCameraPreviewCallback(): Camera.PreviewCallback {
        return object : Camera.PreviewCallback {
            ...
            var currentRed = 0;
            var beepDebounce = false;

            override fun onPreviewFrame(data: ByteArray?, camera: Camera?) {

                Log.d("test", "haiiiiiii")
                ...
                val diff = (currentRed - imgAvg).absoluteValue
                Log.d("imgAvg", imgAvg.toString())
                if (imgAvg == 0 || imgAvg < 40 || diff > 20) {
                ...
                if (imgAvg < currentRed && !beepDebounce) {
                    val bpm = 60000/(System.currentTimeMillis() - lastBeepTime)
                    publishSubject.onNext(Bpm(bpm.toInt(), PulseType.ON))
